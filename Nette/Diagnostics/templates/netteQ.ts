...
module Nette {
    ...
    export class Query {
		...
		// supports cross-browser selectors: #id  |  div  |  div.class  |  .class
		...
		private _find(context, selector?) {
			if (!context || !selector) {
				return [];
		...
		private trav(elem, selector, fce) {
			selector = selector.split('.');
		...
		closest(selector) {
			return this.trav(this[0], selector, 'parentNode');
		}

		prev(selector) {
			return this.trav(this[0] && this[0].previousSibling, selector, 'previousSibling');
		}

		next(selector) {
			return this.trav(this[0] && this[0].nextSibling, selector, 'nextSibling');
		}

		// returns total offset for element
		offset(coords? : Coords) : any {
			if (coords) {
				return this.each(function() {
