...
class PhpWriter extends Nette\Object
{
	...
	public function write($mask)
	{
		...
		$me = $this;
		$mask = Nette\Strings::replace($mask, '#%escape(\(([^()]*+|(?1))+\))#', /*5.2* new Nette\Callback(*/function($m) use ($me) {
			return $me->escape(substr($m[1], 1, -1));
		}/*5.2* )*/);
		$mask = Nette\Strings::replace($mask, '#%modify(\(([^()]*+|(?1))+\))#', /*5.2* new Nette\Callback(*/function($m) use ($me) {
			return $me->formatModifiers(substr($m[1], 1, -1));
		}/*5.2* )*/);

		return Nette\Strings::replace($mask, '#([,+]\s*)?%(node\.word|node\.array|node\.args|var|raw)(\?)?(\s*\+\s*)?()#',
